...
function library:Load(options)
    ...
    local tabCount = #self.tabtoggles + 1
    ...
    local tabtoggle = utility.create("Square", {
        ...
        ZIndex = 6,
        Theme = tabCount == 1 and "Tab Toggle Background" or "Tab Background"
    })
    ...
    local title = utility.create("Text", {
        ...
        Size = 13,
        Position = UDim2.new(0.5, 0, 0, 3),
        Theme = tabCount == 1 and "Text" or "Disabled Text",
        ZIndex = 7,
        Center = true,
        Outline = true,
        Parent = tabtoggle,
    ...
    -- Insert early to calculate layout accurately
    ...
    -- Clean tab button layout after adding
    for i, v in ipairs(self.tabtoggles) do
        v.Size = UDim2.new(1 / #self.tabtoggles, i == 1 and 1 or i == #self.tabtoggles and -2 or -1, 1, 0)
        v.Position = UDim2.new((i - 1) / #self.tabtoggles, i == 1 and 0 or 2, 0, 0)
    end
    ...
    -- Tab toggle interactions
    local mouseover = false

    local function updateColor()
        local themeKey = tab.Visible and "Tab Toggle Background" or "Tab Background"
        local color = tab.Visible
            and (mouseover and utility.changecolor(library.theme[themeKey], 3) or library.theme[themeKey])
            or (mouseover and utility.changecolor(library.theme[themeKey], 3) or library.theme[themeKey])
        tabtoggle.Color = color
    end
    ...
    tabtoggle.MouseButton1Down:Connect(function()
        local themeKey = tab.Visible and "Tab Toggle Background" or "Tab Background"
        tabtoggle.Color = utility.changecolor(library.theme[themeKey], 6)
    end)

    tabtoggle.MouseButton1Click:Connect(function()
        for i, toggle in ipairs(self.tabtoggles) do
            local isActive = (toggle == tabtoggle)
            local tabObj = self.tabs[i]
            local titleObj = self.tabtoggletitles[i]

            tabObj.Visible = isActive
            utility.changeobjecttheme(toggle, isActive and "Tab Toggle Background" or "Tab Background")
            utility.changeobjecttheme(titleObj, isActive and "Text" or "Disabled Text")
        end
        updateColor()
    end)
...
function tabtypes:Section(options)
    ...
    local layout = sectioncontent:AddListLayout(8)

    local function updateSectionHeight()
        section.Size = UDim2.new(1, 0, 0, layout.AbsoluteContentSize.Y + 28)
    end
    ...
    function sectiontypes:Separator(name)
        ...
        local borderOffset = sizeX / 2 + 7

        local separatorborder1 = utility.create("Square", {
            Size = UDim2.new(0, 1, 1, 2),
            Position = UDim2.new(0.5, -borderOffset, 0.5, -1),
            Thickness = 0,
            Filled = true,
            ZIndex = 9,
            Theme = "Object Border",
        ...
        local separatorborder2 = utility.create("Square", {
            Size = UDim2.new(0, 1, 1, 2),
            Position = UDim2.new(0.5, borderOffset - 2, 0, -1),
            Thickness = 0,
            Filled = true,
            ZIndex = 9,
            Theme = "Object Border",
        ...
        local separatorcutoff = utility.create("Square", {
            ...
            Position = UDim2.new(0.5, -sizeX/2 - 6, 0.5, -1),
            ZIndex = 8,
            Filled = true,
            Theme = "Section Background",
            Parent = separator
        ...
        function separatortypes:Set(str)
            local newSizeX = utility.textlength(str, Drawing.Fonts.Plex, 13).X
            local newOffset = newSizeX / 2 + 7

            separatorcutoff.Size = UDim2.new(0, newSizeX + 12, 0, 3)
            separatorcutoff.Position = UDim2.new(0.5, -newOffset, 0.5, -1)
            separatorborder1.Position = UDim2.new(0.5, -newOffset, 0.5, -1)
            separatorborder2.Position = UDim2.new(0.5, newOffset - 2, 0, -1)
